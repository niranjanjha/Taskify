...
const PendingTasks = () => {
  ...
  const handleDelete = useCallback(async (task) => {
    ...
    try {
      // Fix: Correct the endpoint URL - it should be /:id/gp not /:id/gp
      const response = await fetch(`${API_BASE}/${taskId}/gp`, { 
        method: 'DELETE', 
        headers: {
          ...
          sortedPendingTasks.map(task => (
            <TaskItem
              ...
              onToggleComplete={() => handleToggleComplete(
                task._id || task.id,
                !(task.completed || (typeof task.completed === 'string' && task.completed.toLowerCase() === 'yes'))
              )}
