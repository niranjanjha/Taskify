...
import debugFaceRecognitionService from './DebugFaceRecognitionService';

const DebugFaceLogin = ({ onSubmit, onSwitchMode, onSwitchToEmail }) => {
  ...
  useEffect(() => {
    console.log("[DebugFaceLogin] Component mounted");
    const loadModels = async () => {
      if (modelsLoaded || modelsLoading) {
        console.log("[DebugFaceLogin] Models already loaded or loading");
        return;
      }
      
      console.log("[DebugFaceLogin] Starting to load models...");
      setModelsLoading(true);
      try {
        await debugFaceRecognitionService.loadModels();
        console.log("[DebugFaceLogin] Models loaded successfully");
        setModelsLoaded(true);
        setModelsLoading(false);
        toast.success("Face recognition models loaded successfully!");
      } catch (err) {
        console.error("[DebugFaceLogin] Error loading face models:", err);
        setModelsLoading(false);
    ...
    // Cleanup function
    return () => {
      console.log("[DebugFaceLogin] Component unmounting, cleaning up streams");
      if (streamRef.current) {
        streamRef.current.getTracks().forEach(track => {
          console.log("[DebugFaceLogin] Stopping track:", track);
          track.stop();
        });
  ...
  const handleVideoPlay = () => {
    console.log("[DebugFaceLogin] handleVideoPlay called");
    if (videoRef.current && canvasRef.current) {
      ...
      console.log("[DebugFaceLogin] Setting canvas dimensions:", width, "x", height);
      canvas.width = width;
      canvas.height = height;
      
      console.log("[DebugFaceLogin] Video dimensions:", width, "x", height);
      console.log("[DebugFaceLogin] Canvas dimensions set to:", canvas.width, "x", canvas.height);
    } else {
      console.log("[DebugFaceLogin] Video or canvas not available in handleVideoPlay");
    }
  };

  // Start camera for face capture
  const startCamera = async () => {
    console.log("[DebugFaceLogin] startCamera called");
    if (modelsLoading) {
    ...
    try {
      console.log("[DebugFaceLogin] Requesting camera access...");
      ...
      console.log("[DebugFaceLogin] Camera access granted, stream:", stream);
      
      if (videoRef.current) {
        console.log("[DebugFaceLogin] Setting video srcObject");
      ...
      console.error("[DebugFaceLogin] Error accessing camera:", err);
  ...
  // Stop camera
  const stopCamera = () => {
    console.log("[DebugFaceLogin] stopCamera called");
    if (streamRef.current) {
      streamRef.current.getTracks().forEach(track => {
        console.log("[DebugFaceLogin] Stopping track:", track);
        track.stop();
  ...
  const captureAndLogin = async () => {
    console.log("[DebugFaceLogin] captureAndLogin called");
    if (!videoRef.current || !modelsLoaded) {
      console.log("[DebugFaceLogin] Video or models not ready", { 
    ...
    try {
      ...
      console.log("[DebugFaceLogin] Attempting face detection...");
      const detections = canvasRef.current 
        ? await debugFaceRecognitionService.detectFaceWithVisualization(videoRef.current, canvasRef.current)
        : await debugFaceRecognitionService.detectFace(videoRef.current);

      console.log("[DebugFaceLogin] Detection result:", detections);
      ...
      console.log("[DebugFaceLogin] Face detected, proceeding with authentication...");
      ...
      console.log("[DebugFaceLogin] Sending face data to backend:", faceData.length, "elements");
      ...
      console.log("[DebugFaceLogin] Backend response:", data);
      ...
      console.error("[DebugFaceLogin] Login error:", err);
      setDetectionStatus("Authentication failed. Please try again.");
