...
const Layout = ({ user, onLogout }) => {
  ...
  const fetchTasks = useCallback(async () => {
    ...
    try {
      ...
      // FIX: Instead of throwing a frontend error, call onLogout
      // to clear the state/token and trigger the parent component to redirect.
      if (!token) {
        onLogout() 
      ...
      // If the error came from the backend (500, 401, 403)
      if (err.response) {
      ...
      } else if (err.message === "No auth token found") {
          // This case should no longer be hit due to the fix above, but kept as a safeguard.
          onLogout()
      } else {
      ...
      <div className="ml-0 xl:ml-64 lg:ml-64 md:ml-16 pt-16 p-3 sm:p-4 md:p-4 transition-all duration-300">
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-4 sm:gap-6">
          <div className="xl:col-span-2 space-y-3 sm:space-y-4">
            <Outlet context={{ tasks, refreshTasks: fetchTasks, user }} />
          </div>
